...
fid = open("Corrida_Numpy_longdouble.txt", "w") #abro el archivo creado
...
for nc in range(NCOR): #recorro la cantidad de corridas entregado por la variable NCOR
    ...
    for N in Ns: #recorro la lista Ns
        t1 = perf_counter()
        A = laplaciana(N, longdouble) #el hald me tira lo mismo que el float16
        t2 = perf_counter()
        Am1 = inv(A)
        t3 = perf_counter()
...
print(f"Tiempo de inversión - dtype longdouble: {dt_inversion} (ms)")