...
for i in range(NCOR): #recorro la cantidad de numero de corridas 
    fin = f"corrida_{i}_matrizdispersa.txt" #recorro el archivo de texto i creado por timing_matmul  
...
plt.title("Rendimiento INV matriz dispersa") #le escribo el titulo principal a mis graficos
...
plt.ylim(0.1e-3,1000)
plt.legend()
plt.grid() #ploteo la malla del segundo grafico

plt.tight_layout() # se ajusta el grafico para que quede mas bonito y ordenado
plt.savefig("Grafico INV matriz dispersa") #se guarda la foto del grafico en la carpeta donde se está corriendo este programa
