...
fid = open("Corrida_Scipy_longdouble.txt", "w") #abro el archivo creado
...
for nc in range(NCOR): #recorro la cantidad de corridas entregado por la variable NCOR
    ...
    for N in Ns: #recorro la lista Ns
        t1 = perf_counter()
        A = laplaciana(N, dtype = longdouble) #el hald me tira lo mismo que el float16
...
print(f"Uso memoria - dtype longdouble: {bytes_total}")
print(f"Tiempo de inversión dtype longdouble: {dt_inversion} (s)")